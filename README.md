...
*   **📄 BOM (物料清单) 管理**：
    *   **智能匹配**：支持自定义特殊匹配规则，提高元件识别率。
    *   **规则管理**：新增特殊规则管理页面，支持 JSON 格式规则的增删改查、导入导出。
...
*   **🖥️ 现代化界面**：
    ...
    *   支持 **无刷新编辑**，提升操作效率。
    *   固定式侧边栏，不再随页面滚动，支持折叠与记忆用户偏好。
    *   全面引入 FontAwesome 图标，美观易用。
    *   **UTC 时间支持**：后端统一存储 UTC 时间，前端根据用户时区自动转换显示，确保多地协作时间准确。
...
*   `dataset/`: 数据存储目录。
    ...
    *   `special_rules.json`: 用于存储特殊的 BOM 匹配规则。
